...
class DenseGGNNChemModel(ChemModel):
    ...
    def prepare_specific_graph_model(self) -> None:
        ...
        self.placeholders['abs_dist'] = tf.compat.v1.placeholder(tf.float32, [None, struct_dim-1], name='abs_dist') # [b, struct_info] # -1 to remove iteration num
        # iteration number during generation
        self.placeholders['it_num'] = tf.compat.v1.placeholder(tf.int32, [None], name='it_num') 
        # index of structural data for example
        self.placeholders['sdf_idx'] = tf.compat.v1.placeholder(tf.int32, [None], name='sdf_idx')
        # adj for encoder
        ...
        # CNN input
        self.placeholders['cnn_input'] = tf.compat.v1.placeholder(tf.float32, [None, None, None, None, None], name="cnn_input") # [b, n_channels, x, y, z]
    ...
    def get_node_embedding_state(self, one_hot_state, source=False): 
        node_nums=tf.argmax(input=one_hot_state, axis=2)
        if source:
            return tf.nn.embedding_lookup(params=self.weights["node_embedding"], ids=node_nums) * self.ops['graph_state_mask_in']
        else:
            return tf.nn.embedding_lookup(params=self.weights["node_embedding"], ids=node_nums) * self.ops['graph_state_mask_out']
    ...
    def setup_cnn(self, weights, cnn_input, cnn_scope_name):
        with tf.compat.v1.variable_scope(cnn_scope_name) as scope:
            ...
            pool2 = tf.reduce_max(input_tensor=conv2, axis=[2, 3, 4])
    
            flat = tf.compat.v1.layers.flatten(pool2, data_format='channels_first')
    ...
    def compute_mean_and_logvariance(self):
        ...
        avg_last_h_out = tf.reduce_sum(input_tensor=self.ops['final_node_representations_out'] * self.ops['graph_state_mask_out'], axis=1) / \
                         tf.reduce_sum(input_tensor=self.ops['graph_state_mask_out'], axis=1) # [b, 1, h] - mask out unused nodes
    ...
    def sample_with_mean_and_logvariance(self):
        ...
        z_sampled = tf.cond(pred=self.placeholders['is_generative'], true_fn=lambda: z_prior, # standard normal 
                    false_fn=lambda: tf.add(self.ops['mean_out'], tf.multiply(tf.sqrt(tf.exp(self.ops['logvariance_out'])), z_prior))) # non-standard normal
        ...
        inverted_mask = tf.ones(tf.shape(input=self.ops['graph_state_mask_in'])) - self.ops['graph_state_mask_in']
        ...
        atten_mi = tf.reduce_sum(input_tensor=atten_mi, axis=2) / tf.tile(tf.expand_dims(tf.reduce_sum(input_tensor=self.ops['graph_state_mask_out'], axis=1), 1), [1, v, 1])
    ...
    def generate_cross_entropy(self, idx, cross_entropy_losses, edge_predictions, edge_type_predictions):
        ...
        batch_size = tf.shape(input=self.placeholders['initial_node_representation_'+direc])[0]
        ...
        if self.params["use_graph"]:
            if self.params["residual_connection_on"]:
                new_filtered_z_sampled = self.compute_final_node_representations_with_residual(filtered_z_sampled,   
                                                    tf.transpose(a=incre_adj_mat, perm=[1, 0, 2, 3]), 
                                                    "_decoder") # [b, v, h + h]
            else:
        ...
        node_in_focus = tf.reduce_sum(input_tensor=node_sequence * new_filtered_z_sampled, axis=1)# [b, h + h]
        ...
        local_graph_repr_before_expansion = tf.reduce_sum(input_tensor=new_filtered_z_sampled, axis=1) /  \
                                            tf.reduce_sum(input_tensor=self.placeholders['node_mask_'+direc], axis=1, keepdims=True) # [b, h + h]
        ...
        global_graph_repr_before_expansion = tf.reduce_sum(input_tensor=filtered_z_sampled, axis=1) / \
                                            tf.reduce_sum(input_tensor=self.placeholders['node_mask_'+direc], axis=1, keepdims=True)
        ...
        distance_repr = tf.nn.embedding_lookup(params=self.weights['distance_embedding_'+direc_r], ids=distance_to_others) # [b, v, h+h]
        # overlapped edge feature representation
        overlapped_edge_repr = tf.nn.embedding_lookup(params=self.weights['overlapped_edge_weight_'+direc_r], ids=overlapped_edge_features) # [b, v, h+h]
        # concat and reshape.
        ...
        it_num = tf.tile(tf.reshape([tf.cast(idx+self.placeholders['it_num'], tf.float32)], [1, 1]), [tf.shape(input=combined_edge_repr)[0], 1])
        ...
        distance_to_stop_node = tf.nn.embedding_lookup(params=self.weights['distance_embedding_'+direc_r], ids=tf.tile([0], [batch_size]))     # [b, h + h]
        overlap_edge_stop_node = tf.nn.embedding_lookup(params=self.weights['overlapped_edge_weight_'+direc_r], ids=tf.tile([0], [batch_size]))     # [b, h + h]
        ...
        it_num = tf.tile(tf.reshape([tf.cast(idx+self.placeholders['it_num'], tf.float32)], [1, 1]), [tf.shape(input=combined_stop_node_repr)[0], 1])
        # Add cnn featurisation
        ...
        # softmax for edge
        edge_loss =- tf.reduce_sum(input_tensor=tf.math.log(tf.nn.softmax(edge_logits) + SMALL_NUMBER) * edge_labels, axis=1)
        # softmax for edge type 
        edge_type_loss =- edge_type_labels * tf.math.log(edge_type_probs + SMALL_NUMBER) # [b, e, v]
        edge_type_loss = tf.reduce_sum(input_tensor=edge_type_loss, axis=[1, 2]) # [b]
        # total loss
    ...
    def construct_logit_matrices(self):
        v = self.placeholders['num_vertices']
        batch_size=tf.shape(input=self.placeholders['initial_node_representation_out'])[0]
        h_dim = self.params['hidden_size']
        struct_dim = self.params['struct_data_len'] + 1 # 1 for iteration num
        ...
        self.ops['cross_entropy_losses_'+in_direc] = tf.transpose(a=cross_entropy_losses_final, perm=[1,0]) # [b, es]
        ...
        num_atoms = tf.expand_dims(tf.tile(tf.reduce_sum(input_tensor=self.placeholders['node_mask_'+out_direc]-self.placeholders['node_mask_'+in_direc], axis=1, keepdims=True), [1, v]), 2) # [b, v, 1]
        ...
        atten_mi = tf.nn.sigmoid(tf.add(atten_c, atten_yi) + atten_masks)
        atten_mi = tf.reduce_sum(input_tensor=atten_mi, axis=2) / tf.tile(tf.expand_dims(tf.reduce_sum(input_tensor=self.ops['graph_state_mask_'+out_direc], axis=1), 1), [1, v, 1]) # Mask using out_mol not in_mol
    ...
    def construct_loss(self):
        ...
        # Edge loss
        self.ops["edge_loss_"+in_direc] = tf.reduce_sum(input_tensor=self.ops['cross_entropy_losses_'+in_direc] * self.placeholders['iteration_mask_'+out_direc], axis=1)
        ...
        self.ops['kl_loss_'+in_direc] = -0.5 * tf.reduce_sum(input_tensor=kl_loss_in, axis=[1,2]) - 0.5 * tf.reduce_sum(input_tensor=kl_loss_noise, axis=[1,2])
        
        # Node symbol loss
        self.ops['node_symbol_prob_'+in_direc] = tf.nn.softmax(self.ops['node_symbol_logits_'+in_direc])
        self.ops['node_symbol_loss_'+in_direc] = -tf.reduce_sum(input_tensor=tf.math.log(self.ops['node_symbol_prob_'+in_direc] + SMALL_NUMBER) * 
                                                                self.placeholders['node_symbols_'+out_direc], axis=[1,2])

        # Overall losses
        self.ops['mean_edge_loss_'+in_direc] = tf.reduce_mean(input_tensor=self.ops["edge_loss_"+in_direc])
        self.ops['mean_node_symbol_loss_'+in_direc] = tf.reduce_mean(input_tensor=self.ops["node_symbol_loss_"+in_direc])
        self.ops['mean_kl_loss_'+in_direc] = tf.reduce_mean(input_tensor=kl_trade_off_lambda *self.ops['kl_loss_'+in_direc])
        
        return tf.reduce_mean(input_tensor=self.ops["edge_loss_in"] + self.ops['node_symbol_loss_in'] +\
